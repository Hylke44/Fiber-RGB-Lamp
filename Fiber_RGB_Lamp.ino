...
struct {
  ...
  uint8_t g;
  uint8_t b;
}led[4] = { {.r=0, .g=6, .b=3}, {.r=2, .g=1, .b=6}, 
            {.r=5, .g=3, .b=0}, {.r=7, .g=5, .b=1} };
...
void setup() {
  ...
  //set timer0 interrupt at 480Hz
  TCCR0A = 0;// set entire TCCR0A register to 0
  TCCR0B = 0;// same for TCCR0B
  TCNT0  = 0;//initialize counter value to 0
  // set compare match register for 480khz
  OCR0A = 172;// = (16MHz) / (480*256) - 1 (must be <256)
  // turn on CTC mode
  TCCR0A |= (1 << WGM01);
...
void loop() {
  ...
  if (new_frame) {
    ...
    if (counter == 10) {
      ...
      for (i = 0; i < 4; i++) {
        led[i].r++;
        if (led[i].r == 8) led[i].r = 0;
        led[i].g++;
        if (led[i].g == 8) led[i].g = 0;
        led[i].b++;
        if (led[i].b == 8) led[i].b = 0;
      }
